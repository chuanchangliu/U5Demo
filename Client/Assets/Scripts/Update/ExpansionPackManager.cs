...
public class ExpansionPackManager
{
    ...
    {
        m_fileRecord = Path.Combine(Settings.externalPath, Path.Combine("expack", "record.xml"));
        m_pathDownload = Path.Combine(Settings.externalPath, Path.Combine("expack", "pack"));
        m_pathDownloadTemp = Path.Combine(Settings.externalPath, Path.Combine("expack", "temp"));
    }
